...
namespace StockHistory
{
    ...
    class DownloadData
    {
        ...
        {
            List<Task<StockData>> tasks = null;


            //
            // Start tasks to retrieve data from each site:
            //
            Task<StockData> tYahoo = null;
            try
            {
                tYahoo = Task.Factory.StartNew(() =>
                {
            ...
            Task<StockData> tNasdaq = null;
            try
            {
                tNasdaq = Task.Factory.StartNew(() =>
                {
            ...
            Task<StockData> tMsn = null;
            try
            {
                tMsn = Task.Factory.StartNew(() =>
                {
            ...
            //
            tasks = new List<Task<StockData>>() { tYahoo, tNasdaq, tMsn };
            ...
            try
            {
                ...
                } while (result == null && tasks.Count > 0);

                Console.WriteLine($"Task number {index} finished");
                return result;
            }
            catch (Exception ex)
            {
        ...
        private static StockData GetDataFromYahoo(string symbol, int numYearsOfHistory)
        {
            ...
            string url =
                string.Format(
                    "http://ichart.finance.yahoo.com/table.csv?s={0}&d={1}&e={2}&f={3}&g=d&a={1}&b={2}&c={4}&ignore=.csv",
                    symbol,
                    today.Month - 1,
                    today.Day - 1,
                    today.Year,
        ...
        private static StockData GetDataFromMsn(string symbol, int numYearsOfHistory)
        {
            ...
            try
            {
                ...
                string url =
                    string.Format(
                        "http://moneycentral.msn.com/investor/charts/chartdl.aspx?C1=0&C2=1&height=258&width=612&CE=0&symbol={0}&filedownloadbt.x=1",
                        symbol);
